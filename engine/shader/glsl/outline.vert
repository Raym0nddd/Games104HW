...
{
    VulkanMeshInstance selected_instance;
};

layout(set = 0, binding = 2) readonly buffer _unused_name_per_drawcall_vertex_blending
{
    highp mat4 joint_matrices[m_mesh_vertex_blending_max_joint_count];
};
...
void main()
{
    highp mat4  model_matrix           = selected_instance.model_matrix;
    highp float enable_vertex_blending = selected_instance.enable_vertex_blending;
    ...
    if (enable_vertex_blending > 0.0)
    {
        ...
        if (in_weights.x > 0.0 && in_indices.x > 0)
        {
            vertex_blending_matrix +=
                joint_matrices[in_indices.x] * in_weights.x;
        }

        if (in_weights.y > 0.0 && in_indices.y > 0)
        {
            vertex_blending_matrix +=
                joint_matrices[in_indices.y] * in_weights.y;
        }

        if (in_weights.z > 0.0 && in_indices.z > 0)
        {
            vertex_blending_matrix +=
                joint_matrices[in_indices.z] * in_weights.z;
        }

        if (in_weights.w > 0.0 && in_indices.w > 0)
        {
            vertex_blending_matrix +=
                joint_matrices[in_indices.w] * in_weights.w;
        }
